...
import { WorldPanel, type WorldPanelHandlers } from "./world-panel";

export class Panel {
  readonly status: StatusSection;
  readonly settingsForm: SettingsForm;
  readonly worldPanel: WorldPanel;
  ...
  constructor(root: HTMLElement, onGridChanged: () => void, worldHandlers: WorldPanelHandlers) {
    this.status = new StatusSection(API_BASE_URL);
    this.settingsForm = new SettingsForm(onGridChanged);
    this.worldPanel = new WorldPanel(worldHandlers);
